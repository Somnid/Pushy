customElements.define("notification-manager",
    class extends HTMLElement {
        ...
        async connectedCallback() {
            ...
            this.state.hasServiceWorker = !!this.serviceWorker;
            this.state.hasPermission = Notification.permission === "granted";
            this.state.hasSubscription = this.state.hasServiceWorker && !!(await this.serviceWorker.pushManager.getSubscription());
            this.attachEvents();
        }
        createShadowDom() {
        ...
        render() {
            const notificationStatus = (this.state.hasPermission && this.state.isSecure && this.state.hasServiceWorker) 
            ...
            this.dom.enable.disabled = !(this.state.isSecure && this.state.hasServiceWorker);
        }
        ...
        async subscribeForNotifications() {
            try {
                ...
                this.raiseEvent("gotsubscription", subscription);
            } catch (ex) {
